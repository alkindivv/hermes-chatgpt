...
export function launcherTurnControlTimeoutMs(
  remote: boolean,
  phase: LauncherTurnActivity["phase"],
): number {
  if (!remote) {
    return phase === "end"
...
export async function notifyLauncherTurn(
  ...
  throw new Error(
    `Launcher browser control channel ${activity.phase} failed after ${attempts} attempt(s)`
